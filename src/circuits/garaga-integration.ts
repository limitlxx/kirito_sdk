...
export class GaragaMysteryBoxVerifier {
  ...
  // Verification key hashes for different proof types
   ...
   */
  async generateVerifierContract(circuitPath: string, outputPath: string): Promise<void> {
    try {
   ...
   */
  private convertToGaragaFormat(proof: ZKProof, revealType: 'full' | 'bluffing'): GaragaProof {
   ...
   */
  private generateMockVerifierContract(): string {
    return `
...
export class GaragaCLIHelper {
   ...
   */
  static async generateVerificationKey(circuitPath: string): Promise<string> {
   ...
   */
  static async declareContract(contractPath: string): Promise<string> {
   ...
   */
  static async deployContract(classHash: string, constructorArgs: string[]): Promise<string> {
