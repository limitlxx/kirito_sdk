...
export class ZKProofManager {
  ...
  private async generateCircuitProof(circuitInputs: YieldClaimCircuit): Promise<Uint8Array> {
    try {
      ...
      const zkProof = await noirCircuit.generateRevealProof(
        circuitInputs.tokenId,
        circuitInputs.tokenId, // Use tokenId as both box and token for yield claims
        {
          traits: {},
          yieldRange: { min: 0, max: Number(circuitInputs.yieldMultiplier) }
        },
        { type: 'timelock', timestamp: circuitInputs.currentTimestamp },
        Buffer.from(circuitInputs.stakingSecret).toString('hex'),
        'full'
      );
   ...
   */
  private async generateCommitmentProof(circuitInputs: YieldClaimCircuit): Promise<Uint8Array> {
    ...
    const key = await crypto.subtle.importKey(
      'raw',
      new Uint8Array(circuitInputs.stakingSecret),
   ...
   */
  private async verifyCommitmentProof(proof: ZKProof): Promise<boolean> {
    try {
      ...
      const publicInputsData = new TextEncoder().encode(
