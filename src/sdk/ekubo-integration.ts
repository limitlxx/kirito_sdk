...
/**
 * Ekubo API response types
 */
interface EkuboPoolResponse {
  pools: Array<{
    id: string;
    name: string;
    token0: string;
...
interface EkuboPositionResponse {
  positions: Array<{
    pool_id: string;
 ...
 */
export class EkuboIntegration {
  private readonly apiBaseUrl: string;
  private readonly apiKey?: string;
  private readonly starknetAccount: Account;
  ...
  constructor(
    ...
    apiBaseUrl: string = 'https://prod-api.ekubo.org',
    apiKey?: string
  ) {
   ...
   */
  async getLiquidityPools(): Promise<EkuboPool[]> {
    // Check cache first
    const cacheKey = 'pools';
    const cached = this.cache.get<EkuboPool[]>(cacheKey);
    if (cached) {
      return cached;
    }
    ...
    for (let attempt = 1; attempt <= this.maxRetries; attempt++) {
      try {
        const response = await this.makeApiCall<EkuboPoolResponse>('/pools');
   ...
   */
  async getLiquidityPosition(walletAddress: Address, poolId?: string): Promise<EkuboPosition[]> {
    // Check cache first
    const cacheKey = `position_${walletAddress}_${poolId || 'all'}`;
    const cached = this.cache.get<EkuboPosition[]>(cacheKey);
    if (cached) {
      return cached;
    }
    ...
    for (let attempt = 1; attempt <= this.maxRetries; attempt++) {
      try {
        ...
        const response = await this.makeApiCall<EkuboPositionResponse>(endpoint);
        
        const positions = response.positions.map(pos => ({
          poolId: pos.pool_id,
          walletAddress: pos.wallet_address as Address,
   ...
   */
  async addLiquidity(params: EkuboLiquidityParams): Promise<TransactionHash> {
    try {
   ...
   */
  async removeLiquidity(params: EkuboLiquidityParams): Promise<TransactionHash> {
    try {
      // Prepare transaction data for Ekubo DEX contract
      const calldata = [
        params.poolId,
   ...
   */
  async claimTradingFees(walletAddress: Address, poolId: string): Promise<TransactionHash> {
    try {
   ...
   */
  async calculateTradingYield(
    walletAddress: Address,
    poolId: string,
    period: TimePeriod
  ): Promise<EkuboYieldData> {
    try {
      const endpoint = `/yield/${walletAddress}/${poolId}?start=${period.start}&end=${period.end}`;
      const response = await this.makeApiCall<EkuboYieldResponse>(endpoint);
  ...
  async getAggregatedYield(
    ...
    try {
      ...
      const yieldPromises = positions.map(pos => 
        this.calculateTradingYield(walletAddress, pos.poolId, period)
      );
   ...
   */
  async monitorPositionHealth(walletAddress: Address): Promise<{
    totalValue: bigint;
    impermanentLoss: bigint;
    ...
    try {
      ...
      for (const position of positions) {
        ...
        const impermanentLoss = BigInt(
          Math.floor(Number(positionValue) * Math.abs(ilMultiplier))
        );
   ...
   */
  async getOptimalLiquidityPools(
    token0: Address, 
    token1: Address, 
    minLiquidity: bigint
  ): Promise<EkuboPool[]> {
  ...
  async optimizeLiquidityAllocation(
    ...
    allocations: Array<{ poolId: string; token0Amount: bigint; token1Amount: bigint; expectedAPY: number }>;
  ...
  private getMockLiquidityPools(): EkuboPool[] {
    return [
      {
  ...
  private async calculateFallbackYield(
    walletAddress: Address,
    poolId: string,
    period: TimePeriod
  ): Promise<EkuboYieldData> {
    ...
    const tradingFees = BigInt(
      Math.floor(Number(positionValue) * (compoundedMultiplier - 1))
    );
    ...
    const impermanentLoss = BigInt(
      Math.floor(Number(positionValue) * Math.abs(ilFormula))
    );
 ...
 */
export function createEkuboIntegration(
  starknetAccount: Account,
  contractAddress: Address,
  apiBaseUrl?: string,
  apiKey?: string
