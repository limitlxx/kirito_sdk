 ...
 */
export class GardenFinanceBridgeImpl implements GardenFinanceBridge {
   ...
   */
  async wrapBTC(amount: bigint, destinationAddress: Address): Promise<BridgeTransaction> {
    try {
   ...
   */
  async unwrapWBTC(amount: bigint, btcAddress: string): Promise<BridgeTransaction> {
    try {
  ...
  async getOptimalRoute(
    ...
    try {
      ...
      return {
        ...
        fees: route.fees.map((fee: string) => BigInt(fee))
      };
    } catch (error) {
   ...
   */
  async executeAtomicSwapWithRouting(
    fromToken: string,
    toToken: string,
    amount: bigint,
   ...
   */
  async lockSwapFunds(swapId: string, secret: string): Promise<TransactionHash> {
    try {
   ...
   * Redeem atomic swap
   */
  async redeemSwap(swapId: string, secret: string): Promise<TransactionHash> {
    try {
   ...
   * Get swap status
   */
  async getSwapStatus(swapId: string): Promise<AtomicSwap> {
    try {
   ...
   */
  private async monitorTransactionConfirmations(transactionId: string): Promise<void> {
   ...
   */
  private mapStatus(gardenStatus: string): BridgeTransactionStatus {
   ...
   */
  private mapSwapStatus(gardenStatus: string): AtomicSwapStatus {
    switch (gardenStatus.toLowerCase()) {
      case 'created':
        return AtomicSwapStatus.CREATED;
      case 'locked':
        return AtomicSwapStatus.LOCKED;
      case 'redeemed':
        return AtomicSwapStatus.REDEEMED;
      case 'refunded':
        return AtomicSwapStatus.REFUNDED;
      default:
        return AtomicSwapStatus.CREATED;
    }
  }
   ...
   */
  private getChainForToken(token: string): string {
   ...
   */
  private getRequiredConfirmations(token: string): number {
    switch (token.toLowerCase()) {
      case 'btc':
        return 6;
      case 'eth':
 ...
 */
export function createGardenFinanceBridge(
  network: 'mainnet' | 'testnet' = 'testnet',
  apiKey?: string
): GardenFinanceBridge {
