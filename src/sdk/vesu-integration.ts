...
import { Address, TokenId, TransactionHash, YieldAmount, TimePeriod } from '../types';
...
interface VesuPositionResponse {
  positions: Array<{
    pool_id: string;
...
export class VesuIntegration {
  ...
  constructor(
    apiBaseUrl: string = 'https://api.vesu.xyz',
   ...
   */
  async getLendingPools(): Promise<VesuPool[]> {
    const maxRetries = 3;
    const retryDelay = 1000; // 1 second
    
    for (let attempt = 1; attempt <= maxRetries; attempt++) {
      try {
        const response = await this.makeApiCall<VesuPoolResponse>('/pools');
        
        return response.pools.map(pool => ({
          id: pool.id,
          name: pool.name,
          asset: pool.asset as Address,
   ...
   */
  async getLendingPosition(walletAddress: Address, poolId?: string): Promise<VesuPosition[]> {
    const maxRetries = 3;
    const retryDelay = 1000; // 1 second
    ...
    for (let attempt = 1; attempt <= maxRetries; attempt++) {
      try {
        ...
        const response = await this.makeApiCall<VesuPositionResponse>(endpoint);
   ...
   */
  async depositToLendingPool(params: VesuLendingParams): Promise<TransactionHash> {
    try {
   ...
   */
  async withdrawFromLendingPool(params: VesuLendingParams): Promise<TransactionHash> {
    try {
      // Prepare transaction data for Vesu lending contract
      const calldata = [
        params.poolId,
   ...
   */
  async calculateLendingYield(
    walletAddress: Address,
    poolId: string,
    period: TimePeriod
  ): Promise<VesuYieldData> {
    try {
      const endpoint = `/yield/${walletAddress}/${poolId}?start=${period.start}&end=${period.end}`;
      const response = await this.makeApiCall<VesuYieldResponse>(endpoint);
  ...
  async getAggregatedYield(
    ...
    try {
      ...
      const yieldPromises = positions.map(pos => 
        this.calculateLendingYield(walletAddress, pos.poolId, period)
      );
   ...
   */
  async getOptimalLendingPools(asset: Address, amount: bigint): Promise<VesuPool[]> {
    try {
  ...
  async optimizeYieldAllocation(
    ...
    allocations: Array<{ poolId: string; amount: bigint; expectedAPY: number }>;
  ...
  private getMockLendingPools(): VesuPool[] {
    return [
      {
        id: 'vesu_eth_pool',
  ...
  private async calculateFallbackYield(
    ...
    const supplyYield = BigInt(
      Math.floor(Number(position.suppliedAmount) * (compoundedSupplyMultiplier - 1))
    );
    ...
    const borrowCost = BigInt(
      Math.floor(Number(position.borrowedAmount) * (compoundedBorrowMultiplier - 1))
    );
 ...
 */
export function createVesuIntegration(
  starknetAccount: Account,
  contractAddress: Address,
  apiBaseUrl?: string,
  apiKey?: string
