...
async function createCollectionGIFFromImages(
  imagesDir: string,
  outputPath: string,
  options: {
    width?: number;
  ...
  try {
    ...
    const nftDirs = readdirSync(imagesDir, { withFileTypes: true })
    ...
    for (const nftDir of nftDirs) {
      const imagePath = join(imagesDir, nftDir, pattern);
      try {
...
async function createMultipleCollectionGIFs() {
  ...
  for (const variant of variants) {
    try {
      ...
      const outputPath = join(baseDir, `${variant.name}.gif`);
