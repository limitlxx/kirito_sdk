...
exports.generateNFTsFromTraits = generateNFTsFromTraits;
...
async function generateNFTsFromTraits() {
    ...
    try {
        ...
        const config = hashlips_engine_1.HashLipsEngine.createHashLipsConfig(traitsPath, 10, {
            rarityDelimiter: '#',
            debugLogs: true,
        ...
        const engine = new hashlips_engine_1.HashLipsEngine(compositeOptions);
        ...
        for (let i = 0; i < nfts.length; i++) {
            const nft = nfts[i];
            const nftDir = (0, path_1.join)(outputDir, `nft-${nft.tokenId}`);
            if (!(0, fs_1.existsSync)(nftDir)) {
                (0, fs_1.mkdirSync)(nftDir, { recursive: true });
            }
            ...
            try {
                ...
                const variantResults = await engine.generateNFTWithVariants(parseInt(nft.tokenId), config.layers, variants);
                // Save all variants
                let savedCount = 0;
                for (const [variantName, buffer] of Object.entries(variantResults.variants)) {
                    ...
                    const filepath = (0, path_1.join)(nftDir, filename);
                    (0, fs_1.writeFileSync)(filepath, buffer);
                    console.log(`     ✅ ${filename} (${buffer.length} bytes)`);
                    savedCount++;
                }
                // Save metadata
                const metadataPath = (0, path_1.join)(nftDir, 'metadata.json');
                const metadata = {
                    ...nft.metadata,
        ...
        try {
            ...
            const collectionGIFPath = (0, path_1.join)(outputDir, 'collection-thumbnail.gif');
            (0, fs_1.writeFileSync)(collectionGIFPath, collectionGIF);
        ...
        const summaryPath = (0, path_1.join)(outputDir, 'collection-summary.json');
        (0, fs_1.writeFileSync)(summaryPath, JSON.stringify(collectionSummary, null, 2));
...
## Layer Structure
${config.layers.map(layer => `### ${layer.name}\n${layer.traits.map(trait => `- **${trait.name}** (weight: ${trait.weight})`).join('\n')}`).join('\n\n')}

## Rarity Distribution
${Object.entries(stats.rarityDistribution).map(([range, count]) => `- **${range}**: ${count} NFTs`).join('\n')}
...
`;
        const readmePath = (0, path_1.join)(outputDir, 'README.md');
        (0, fs_1.writeFileSync)(readmePath, readmeContent);
...
//# sourceMappingURL=generate-nfts-from-traits.js.map