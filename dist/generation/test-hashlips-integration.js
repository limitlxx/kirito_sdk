...
exports.testHashLipsIntegration = testHashLipsIntegration;
...
async function testHashLipsIntegration() {
    ...
    try {
        ...
        // Create directories
        (0, fs_1.mkdirSync)((0, path_1.join)(testLayersDir, '01_Background'), { recursive: true });
        (0, fs_1.mkdirSync)((0, path_1.join)(testLayersDir, '02_Character'), { recursive: true });
        (0, fs_1.mkdirSync)((0, path_1.join)(testLayersDir, '03_Accessory'), { recursive: true });
        ...
        const traitFiles = [
            // Background layer
            { path: (0, path_1.join)(testLayersDir, '01_Background', 'Blue#70.png'), buffer: testImageBuffer },
            { path: (0, path_1.join)(testLayersDir, '01_Background', 'Red#20.png'), buffer: testImageBuffer },
            { path: (0, path_1.join)(testLayersDir, '01_Background', 'Green#10.png'), buffer: testImageBuffer },
            // Character layer
            { path: (0, path_1.join)(testLayersDir, '02_Character', 'Warrior#50.png'), buffer: testImageBuffer },
            { path: (0, path_1.join)(testLayersDir, '02_Character', 'Mage#30.png'), buffer: testImageBuffer },
            { path: (0, path_1.join)(testLayersDir, '02_Character', 'Rogue#20.png'), buffer: testImageBuffer },
            // Accessory layer
            { path: (0, path_1.join)(testLayersDir, '03_Accessory', 'Sword#40.png'), buffer: testImageBuffer },
            { path: (0, path_1.join)(testLayersDir, '03_Accessory', 'Staff#35.png'), buffer: testImageBuffer },
            { path: (0, path_1.join)(testLayersDir, '03_Accessory', 'Dagger#25.png'), buffer: testImageBuffer }
        ];
        // Write trait files
        for (const file of traitFiles) {
            (0, fs_1.writeFileSync)(file.path, file.buffer);
        }
        ...
        const layers = hashlips_engine_1.HashLipsEngine.loadLayersFromDirectory(testLayersDir);
        ...
        const config = hashlips_engine_1.HashLipsEngine.createHashLipsConfig(testLayersDir, 10, {
            rarityDelimiter: '#',
            debugLogs: true,
            namePrefix: 'Test NFT',
        ...
        for (const filename of testFilenames) {
            const parsed = hashlips_engine_1.HashLipsEngine.parseTraitFilename(filename, '#');
        ...
        const variants = hashlips_engine_1.HashLipsEngine.createHashLipsVariants();
        const firstNFT = nfts[0];
        const nftWithVariants = await engine.generateNFTWithVariants(parseInt(firstNFT.tokenId), config.layers, variants);
...
//# sourceMappingURL=test-hashlips-integration.js.map