...
exports.testRealNFTGeneration = testRealNFTGeneration;
...
async function createSampleTraitsFolder() {
    ...
    for (const layer of layers) {
        const layerDir = (0, path_1.join)(traitsDir, layer.name);
        (0, fs_1.mkdirSync)(layerDir, { recursive: true });
        for (const traitName of layer.traits) {
            const [name, weight] = traitName.split('#');
            const filename = `${traitName}.png`;
            const filepath = (0, path_1.join)(layerDir, filename);
            // Get color for this trait
            const color = colors[name] || [128, 128, 128];
            const pngBuffer = createColoredPNG(color[0], color[1], color[2]);
            (0, fs_1.writeFileSync)(filepath, pngBuffer);
...
async function testRealNFTGeneration() {
    ...
    try {
        ...
        const config = hashlips_engine_1.HashLipsEngine.createHashLipsConfig(traitsDir, 5, {
            rarityDelimiter: '#',
            debugLogs: true,
        ...
        const variantResults = await engine.generateNFTWithVariants(parseInt(firstNFT.tokenId), config.layers, variants);
        ...
        for (const [name, buffer] of Object.entries(variantResults.variants)) {
            ...
            (0, fs_1.writeFileSync)((0, path_1.join)(outputDir, filename), buffer);
            console.log(`  Saved: ${filename}`);
        }
        // Save metadata
        const metadataFile = (0, path_1.join)(outputDir, 'nft-1-metadata.json');
        (0, fs_1.writeFileSync)(metadataFile, JSON.stringify(firstNFT.metadata, null, 2));
...
//# sourceMappingURL=test-real-traits.js.map