...
exports.XverseBridgeImpl = void 0;
exports.createXverseBridge = createXverseBridge;
...
class XverseBridgeImpl {
    ...
    async connectWallet() {
        try {
            ...
            if (!window.XverseProviders) {
                throw new Error('Xverse wallet extension not detected. Please install Xverse from https://www.xverse.app/');
            }
            ...
            const timeoutPromise = new Promise((_, reject) => setTimeout(() => reject(new Error('Wallet connection timeout')), connectionTimeout));
            ...
            const paymentAddress = response.find((addr) => addr.purpose === 'payment');
     ...
     */
    async bridgeToStarknet(token, amount, starknetAddress) {
        if (!this.wallet) {
        ...
        try {
            ...
            return {
                id: bridge.bridge_id,
                status: bridge_1.BridgeTransactionStatus.PENDING,
                fromChain: 'bitcoin',
                toChain: 'starknet',
                fromToken: token,
     ...
     */
    async bridgeMultipleTokens(tokens, starknetAddress) {
        if (!this.wallet) {
            throw new Error('Wallet not connected. Call connectWallet() first.');
        }
        const transactions = [];
     ...
     */
    async estimateBridgeFees(token, amount, destination) {
        try {
     ...
     */
    async convertETHToWBTC(ethAmount, starknetAddress) {
        return this.bridgeToStarknet('ETH', ethAmount, starknetAddress);
    }
    /**
     * Convert STRK to WBTC via Xverse bridge
     */
    async convertSTRKToWBTC(strkAmount, starknetAddress) {
        return this.bridgeToStarknet('STRK', strkAmount, starknetAddress);
    }
    /**
     * Convert USDC to WBTC via Xverse bridge
     */
    async convertUSDCToWBTC(usdcAmount, starknetAddress) {
        return this.bridgeToStarknet('USDC', usdcAmount, starknetAddress);
    }
    /**
     * Optimize bridge fees by batching transactions
     */
    async optimizeBridgeFees(tokens, starknetAddress) {
        // Get individual estimates
        const individualEstimates = await Promise.all(tokens.map(({ token, amount }) => this.estimateBridgeFees(token, amount, starknetAddress)));
        const totalIndividualFees = individualEstimates.reduce((sum, estimate) => sum + estimate.totalFee, 0n);
        // Try batched transaction
        try {
            const batchedTransactions = await this.bridgeMultipleTokens(tokens, starknetAddress);
            const batchedFees = await Promise.all(batchedTransactions.map(tx => this.estimateBridgeFees(tx.fromToken, tx.fromAmount, starknetAddress)));
            const totalBatchedFees = batchedFees.reduce((sum, estimate) => sum + estimate.totalFee, 0n);
            return {
            ...
            const individualTransactions = await Promise.all(tokens.map(({ token, amount }) => this.bridgeToStarknet(token, amount, starknetAddress)));
            return {
    ...
    async signTransaction(unsignedTx) {
        ...
        try {
            ...
            const timeoutPromise = new Promise((_, reject) => setTimeout(() => reject(new Error('Transaction signing timeout')), signingTimeout));
     ...
     */
    getDestinationToken(sourceToken) {
...
exports.XverseBridgeImpl = XverseBridgeImpl;
/**
 * Factory function to create Xverse bridge instance
 */
function createXverseBridge(network = 'testnet') {
    return new XverseBridgeImpl(network);
}
//# sourceMappingURL=xverse-bridge.js.map