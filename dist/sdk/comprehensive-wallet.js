...
exports.ComprehensiveWallet = void 0;
exports.createComprehensiveWallet = createComprehensiveWallet;
/**
...
class ComprehensiveWallet {
    constructor(config, walletAddress, tokenId) {
        this.config = config;
     ...
     */
    async transferToken(tokenAddress, recipient, amount, memo) {
        try {
            // Validate inputs
            ...
            const txHash = await this.executeWalletTransaction(this.config.network.contracts.multiTokenWallet, 'transfer_token', transferData);
     ...
     */
    async swapTokens(fromToken, toToken, amount, minOutput, slippageTolerance = 0.5) {
        try {
            ...
            const txHash = await this.executeWalletTransaction(route.route[0], // First DEX in route
            'swap_exact_tokens_for_tokens', swapData);
     ...
     */
    async stakeTokens(protocol, tokenAddress, amount, lockPeriod) {
        try {
            ...
            const txHash = await this.executeWalletTransaction(protocol.address, 'stake', stakeData);
     ...
     */
    async unstakeTokens(protocol, tokenAddress, amount) {
        try {
            ...
            const txHash = await this.executeWalletTransaction(protocol.address, 'unstake', unstakeData);
     ...
     */
    async executeBatchTransactions(operations) {
        try {
            ...
            const txHash = await this.executeWalletTransaction(this.config.network.contracts.multiTokenWallet, 'execute_batch', batchData);
     ...
     */
    async getOptimalSwapRoute(fromToken, toToken, amount) {
        try {
    ...
    async getStakingPosition(protocolAddress, tokenAddress) {
    ...
    async getAvnuRoute(fromToken, toToken, amount) {
        try {
    ...
    async getFibrousRoute(fromToken, toToken, amount) {
        try {
    ...
    async getEkuboRoute(fromToken, toToken, amount) {
        try {
    ...
    encodeTransferData(tokenAddress, recipient, amount) {
        return [tokenAddress, recipient, amount.toString()];
    }
    encodeSwapData(route, minOutput) {
        return [
            route.fromToken,
            route.toToken,
    ...
    encodeStakeData(tokenAddress, amount, lockPeriod) {
        return [tokenAddress, amount.toString(), lockPeriod || 0];
    }
    encodeUnstakeData(tokenAddress, amount) {
        return [tokenAddress, amount.toString()];
    }
    encodeBatchData(operations, order) {
    ...
    async executeWalletTransaction(contractAddress, method, params) {
    ...
    async callWalletView(method, params) {
        // Mock implementation - would use Starknet.js
...
exports.ComprehensiveWallet = ComprehensiveWallet;
/**
 * Factory function to create comprehensive wallet
 */
function createComprehensiveWallet(config, walletAddress, tokenId) {
    return new ComprehensiveWallet(config, walletAddress, tokenId);
}
//# sourceMappingURL=comprehensive-wallet.js.map